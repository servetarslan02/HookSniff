...
async fn main() -> Result<()> {
    ...
    // ═══════════════════════════════════════════════════════
    // CORTEX — Akıllı Sistem Motoru (9 Aşama)
    // ═══════════════════════════════════════════════════════

    // Stage 1: Hourly Stats Aggregation (her saat 00:01'de)
    let cortex_hourly_pool = pool.clone();
    tokio::spawn(async move {
        loop {
            // Sleep 55 minutes then check if it's near the hour mark
            tokio::time::sleep(std::time::Duration::from_secs(55 * 60)).await;

            let now = chrono::Utc::now();
            // Wait until we're past XX:01
            let secs_in_hour = now.timestamp() % 3600;
            if secs_in_hour < 60 {
                // We're within the first minute of the hour, run aggregation
            } else {
                // Wait until next hour + 1 min
                let wait_secs = 3660 - secs_in_hour;
                tokio::time::sleep(std::time::Duration::from_secs(wait_secs as u64)).await;
            }

            if !hooksniff_api::cortex::try_cortex_lock(&cortex_hourly_pool, "cortex_hourly", 300).await {
                continue;
            }
            let now2 = chrono::Utc::now();
            let hour_start_secs = (now2.timestamp() / 3600 - 1) * 3600;
            let hour_start = chrono::DateTime::from_timestamp(hour_start_secs, 0).unwrap_or(now2);
            match hooksniff_api::cortex::signal_collector::aggregate_hourly_stats(&cortex_hourly_pool, hour_start).await {
                Ok(n) => {
                    tracing::info!("📊 Cortex Hourly: {} endpoints aggregated", n);
                    hooksniff_api::cortex::CORTEX_METRICS.hourly_stats_runs.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
                    hooksniff_api::cortex::CORTEX_METRICS.hourly_stats_endpoints.fetch_add(n, std::sync::atomic::Ordering::Relaxed);
                }
                Err(e) => {
                    tracing::error!("❌ Cortex Hourly failed: {:?}", e);
                    hooksniff_api::cortex::CORTEX_METRICS.hourly_stats_errors.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
                }
            }
            hooksniff_api::cortex::release_cortex_lock(&cortex_hourly_pool, "cortex_hourly").await;
        }
    });

    // Stage 2: Profile Update (her 15 dakika)
    let cortex_profile_pool = pool.clone();
    tokio::spawn(async move {
        loop {
            let config = hooksniff_api::cortex::CortexConfig::load(&cortex_profile_pool).await;
            tokio::time::sleep(std::time::Duration::from_secs((config.profile_update_interval_mins * 60) as u64)).await;

            if !hooksniff_api::cortex::try_cortex_lock(&cortex_profile_pool, "cortex_profile", 600).await {
                continue;
            }
            match hooksniff_api::cortex::profile_engine::update_all_profiles(&cortex_profile_pool).await {
                Ok(n) => {
                    tracing::info!("📊 Cortex Profile: {} profiles updated", n);
                    hooksniff_api::cortex::CORTEX_METRICS.profile_updates.fetch_add(n, std::sync::atomic::Ordering::Relaxed);
                }
                Err(e) => tracing::error!("❌ Cortex Profile failed: {:?}", e),
            }
            hooksniff_api::cortex::release_cortex_lock(&cortex_profile_pool, "cortex_profile").await;
        }
    });

    // Stage 3: Anomaly Scoring + Correlation (her 5 dakika)
    let cortex_anomaly_pool = pool.clone();
    tokio::spawn(async move {
        loop {
            tokio::time::sleep(std::time::Duration::from_secs(5 * 60)).await;

            if !hooksniff_api::cortex::try_cortex_lock(&cortex_anomaly_pool, "cortex_anomaly", 300).await {
                continue;
            }
            let config = hooksniff_api::cortex::CortexConfig::load(&cortex_anomaly_pool).await;
            // Score all active endpoints
            let endpoints: Vec<(uuid::Uuid,)> = sqlx::query_as(
                "SELECT id FROM endpoints WHERE is_active = true"
            ).fetch_all(&cortex_anomaly_pool).await.unwrap_or_default();
            for (eid,) in endpoints {
                if let Ok(Some(result)) = hooksniff_api::cortex::anomaly_scorer::score_endpoint(&cortex_anomaly_pool, eid, &config).await {
                    if result.score > config.anomaly_high_threshold {
                        hooksniff_api::cortex::CORTEX_METRICS.anomaly_scores_high.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
                    }
                }
            }
            // Correlate alerts
            if let Ok(Some(_)) = hooksniff_api::cortex::alert_correlation::correlate_alerts(&cortex_anomaly_pool, &config).await {
                hooksniff_api::cortex::CORTEX_METRICS.alerts_correlated.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            }
            hooksniff_api::cortex::release_cortex_lock(&cortex_anomaly_pool, "cortex_anomaly").await;
        }
    });
    ...
    tokio::spawn(async move {
        loop {
            ...
            let config = hooksniff_api::cortex::CortexConfig::load(&cortex_healing_pool).await;
            match hooksniff_api::cortex::healing_engine::run_healing(&cortex_healing_pool, &config).await {
                Ok(n) => {
                    if n > 0 { tracing::info!("🔧 Cortex Healing: {} actions taken", n); }
                }
                Err(e) => tracing::error!("❌ Cortex Healing failed: {:?}", e),
            }
            hooksniff_api::cortex::release_cortex_lock(&cortex_healing_pool, "cortex_healing").await;
        }
    });
    ...
    tokio::spawn(async move {
        loop {
            ...
            let config = hooksniff_api::cortex::CortexConfig::load(&cortex_predict_pool).await;
            match hooksniff_api::cortex::predictive_engine::run_predictions(&cortex_predict_pool, &config).await {
                Ok(n) => {
                    if n > 0 { tracing::info!("🔮 Cortex Predict: {} predictions generated", n); }
                }
                Err(e) => tracing::error!("❌ Cortex Predict failed: {:?}", e),
            }
            hooksniff_api::cortex::release_cortex_lock(&cortex_predict_pool, "cortex_predict").await;
        }
    });

    // Stage 8: Insights (her gün)
    let cortex_insights_pool = pool.clone();
    tokio::spawn(async move {
        loop {
            tokio::time::sleep(std::time::Duration::from_secs(24 * 60 * 60)).await;

            if !hooksniff_api::cortex::try_cortex_lock(&cortex_insights_pool, "cortex_report", 3600).await {
                continue;
            }
            let config = hooksniff_api::cortex::CortexConfig::load(&cortex_insights_pool).await;
            match hooksniff_api::cortex::insights_engine::generate_insights(&cortex_insights_pool, &config).await {
                Ok(n) => {
                    if n > 0 { tracing::info!("💡 Cortex Insights: {} generated", n); }
                }
                Err(e) => tracing::error!("❌ Cortex Insights failed: {:?}", e),
            }
            hooksniff_api::cortex::release_cortex_lock(&cortex_insights_pool, "cortex_report").await;
        }
    });
