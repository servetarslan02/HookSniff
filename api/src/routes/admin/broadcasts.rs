...
pub async fn update_broadcast(
    ...
    .bind(req.link.as_ref().map(|o| o.as_deref().filter(|s| !s.trim().is_empty())))
    .bind(req.link_text.as_ref().map(|o| o.as_deref().filter(|s| !s.trim().is_empty())))
    .bind(req.target_plan.as_ref().map(|o| o.as_deref()))
    .bind(req.is_active)
    .bind(req.starts_at)
    .bind(req.expires_at)
    .bind(id)
