/*
HookSniff API

Webhook delivery, monitoring, and management API. All endpoints under `/v1` require authentication via `Authorization: Bearer <api_key>` header unless marked as **Public**. 

API version: 1.0.0
Contact: support@hooksniff.vercel.app
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package hooksniff

import (
	"encoding/json"
)

// checks if the AdminFeatureFlagsIdPutRequest type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &AdminFeatureFlagsIdPutRequest{}

// AdminFeatureFlagsIdPutRequest struct for AdminFeatureFlagsIdPutRequest
type AdminFeatureFlagsIdPutRequest struct {
	Name *string `json:"name,omitempty"`
	Description *string `json:"description,omitempty"`
	IsEnabled *bool `json:"is_enabled,omitempty"`
	RolloutPercentage *int32 `json:"rollout_percentage,omitempty"`
	EnabledForPlans []string `json:"enabled_for_plans,omitempty"`
}

// NewAdminFeatureFlagsIdPutRequest instantiates a new AdminFeatureFlagsIdPutRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewAdminFeatureFlagsIdPutRequest() *AdminFeatureFlagsIdPutRequest {
	this := AdminFeatureFlagsIdPutRequest{}
	return &this
}

// NewAdminFeatureFlagsIdPutRequestWithDefaults instantiates a new AdminFeatureFlagsIdPutRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewAdminFeatureFlagsIdPutRequestWithDefaults() *AdminFeatureFlagsIdPutRequest {
	this := AdminFeatureFlagsIdPutRequest{}
	return &this
}

// GetName returns the Name field value if set, zero value otherwise.
func (o *AdminFeatureFlagsIdPutRequest) GetName() string {
	if o == nil || IsNil(o.Name) {
		var ret string
		return ret
	}
	return *o.Name
}

// GetNameOk returns a tuple with the Name field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AdminFeatureFlagsIdPutRequest) GetNameOk() (*string, bool) {
	if o == nil || IsNil(o.Name) {
		return nil, false
	}
	return o.Name, true
}

// HasName returns a boolean if a field has been set.
func (o *AdminFeatureFlagsIdPutRequest) HasName() bool {
	if o != nil && !IsNil(o.Name) {
		return true
	}

	return false
}

// SetName gets a reference to the given string and assigns it to the Name field.
func (o *AdminFeatureFlagsIdPutRequest) SetName(v string) {
	o.Name = &v
}

// GetDescription returns the Description field value if set, zero value otherwise.
func (o *AdminFeatureFlagsIdPutRequest) GetDescription() string {
	if o == nil || IsNil(o.Description) {
		var ret string
		return ret
	}
	return *o.Description
}

// GetDescriptionOk returns a tuple with the Description field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AdminFeatureFlagsIdPutRequest) GetDescriptionOk() (*string, bool) {
	if o == nil || IsNil(o.Description) {
		return nil, false
	}
	return o.Description, true
}

// HasDescription returns a boolean if a field has been set.
func (o *AdminFeatureFlagsIdPutRequest) HasDescription() bool {
	if o != nil && !IsNil(o.Description) {
		return true
	}

	return false
}

// SetDescription gets a reference to the given string and assigns it to the Description field.
func (o *AdminFeatureFlagsIdPutRequest) SetDescription(v string) {
	o.Description = &v
}

// GetIsEnabled returns the IsEnabled field value if set, zero value otherwise.
func (o *AdminFeatureFlagsIdPutRequest) GetIsEnabled() bool {
	if o == nil || IsNil(o.IsEnabled) {
		var ret bool
		return ret
	}
	return *o.IsEnabled
}

// GetIsEnabledOk returns a tuple with the IsEnabled field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AdminFeatureFlagsIdPutRequest) GetIsEnabledOk() (*bool, bool) {
	if o == nil || IsNil(o.IsEnabled) {
		return nil, false
	}
	return o.IsEnabled, true
}

// HasIsEnabled returns a boolean if a field has been set.
func (o *AdminFeatureFlagsIdPutRequest) HasIsEnabled() bool {
	if o != nil && !IsNil(o.IsEnabled) {
		return true
	}

	return false
}

// SetIsEnabled gets a reference to the given bool and assigns it to the IsEnabled field.
func (o *AdminFeatureFlagsIdPutRequest) SetIsEnabled(v bool) {
	o.IsEnabled = &v
}

// GetRolloutPercentage returns the RolloutPercentage field value if set, zero value otherwise.
func (o *AdminFeatureFlagsIdPutRequest) GetRolloutPercentage() int32 {
	if o == nil || IsNil(o.RolloutPercentage) {
		var ret int32
		return ret
	}
	return *o.RolloutPercentage
}

// GetRolloutPercentageOk returns a tuple with the RolloutPercentage field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AdminFeatureFlagsIdPutRequest) GetRolloutPercentageOk() (*int32, bool) {
	if o == nil || IsNil(o.RolloutPercentage) {
		return nil, false
	}
	return o.RolloutPercentage, true
}

// HasRolloutPercentage returns a boolean if a field has been set.
func (o *AdminFeatureFlagsIdPutRequest) HasRolloutPercentage() bool {
	if o != nil && !IsNil(o.RolloutPercentage) {
		return true
	}

	return false
}

// SetRolloutPercentage gets a reference to the given int32 and assigns it to the RolloutPercentage field.
func (o *AdminFeatureFlagsIdPutRequest) SetRolloutPercentage(v int32) {
	o.RolloutPercentage = &v
}

// GetEnabledForPlans returns the EnabledForPlans field value if set, zero value otherwise.
func (o *AdminFeatureFlagsIdPutRequest) GetEnabledForPlans() []string {
	if o == nil || IsNil(o.EnabledForPlans) {
		var ret []string
		return ret
	}
	return o.EnabledForPlans
}

// GetEnabledForPlansOk returns a tuple with the EnabledForPlans field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AdminFeatureFlagsIdPutRequest) GetEnabledForPlansOk() ([]string, bool) {
	if o == nil || IsNil(o.EnabledForPlans) {
		return nil, false
	}
	return o.EnabledForPlans, true
}

// HasEnabledForPlans returns a boolean if a field has been set.
func (o *AdminFeatureFlagsIdPutRequest) HasEnabledForPlans() bool {
	if o != nil && !IsNil(o.EnabledForPlans) {
		return true
	}

	return false
}

// SetEnabledForPlans gets a reference to the given []string and assigns it to the EnabledForPlans field.
func (o *AdminFeatureFlagsIdPutRequest) SetEnabledForPlans(v []string) {
	o.EnabledForPlans = v
}

func (o AdminFeatureFlagsIdPutRequest) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o AdminFeatureFlagsIdPutRequest) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Name) {
		toSerialize["name"] = o.Name
	}
	if !IsNil(o.Description) {
		toSerialize["description"] = o.Description
	}
	if !IsNil(o.IsEnabled) {
		toSerialize["is_enabled"] = o.IsEnabled
	}
	if !IsNil(o.RolloutPercentage) {
		toSerialize["rollout_percentage"] = o.RolloutPercentage
	}
	if !IsNil(o.EnabledForPlans) {
		toSerialize["enabled_for_plans"] = o.EnabledForPlans
	}
	return toSerialize, nil
}

type NullableAdminFeatureFlagsIdPutRequest struct {
	value *AdminFeatureFlagsIdPutRequest
	isSet bool
}

func (v NullableAdminFeatureFlagsIdPutRequest) Get() *AdminFeatureFlagsIdPutRequest {
	return v.value
}

func (v *NullableAdminFeatureFlagsIdPutRequest) Set(val *AdminFeatureFlagsIdPutRequest) {
	v.value = val
	v.isSet = true
}

func (v NullableAdminFeatureFlagsIdPutRequest) IsSet() bool {
	return v.isSet
}

func (v *NullableAdminFeatureFlagsIdPutRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableAdminFeatureFlagsIdPutRequest(val *AdminFeatureFlagsIdPutRequest) *NullableAdminFeatureFlagsIdPutRequest {
	return &NullableAdminFeatureFlagsIdPutRequest{value: val, isSet: true}
}

func (v NullableAdminFeatureFlagsIdPutRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableAdminFeatureFlagsIdPutRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


