...
/**
 * Official Java client for the HookRelay webhook delivery service.
 *
 * <pre>{@code
 * HookRelayClient client = new HookRelayClient("hr_live_...");
 *
 * // Create endpoint
...
public class HookRelayClient {
     ...
     *
     * @param apiKey Your HookRelay API key
     */
    ...
    /**
     * Create a new HookRelay client with custom configuration.
     *
     * @param apiKey  Your HookRelay API key
     * @param baseUrl API base URL
     * @param timeout Request timeout in milliseconds
     */
    public HookRelayClient(String apiKey, String baseUrl, int timeout) {
        this.apiKey = apiKey;
    ...
    JsonObject request(String method, String path, JsonObject body) {
        try {
            ...
            switch (statusCode) {
                case 400: throw new HookRelayException.ValidationException(message);
                case 401: throw new HookRelayException.AuthenticationException(message);
                case 404: throw new HookRelayException.NotFoundException(message);
                case 413: throw new HookRelayException.PayloadTooLargeException(message);
                case 429: throw new HookRelayException.RateLimitException(message);
                default: throw new HookRelayException(message, statusCode, "UNKNOWN");
            }
        } catch (HookRelayException e) {
            throw e;
        } catch (Exception e) {
            throw new HookRelayException("Request failed: " + e.getMessage(), 0, "NETWORK_ERROR");
        }
    }
