/**
 * HookSniff API
 * Webhook delivery, monitoring, and management API. All endpoints under `/v1` require authentication via `Authorization: Bearer <api_key>` header unless marked as **Public**. 
 *
 * The version of the OpenAPI document: 1.0.0
 * Contact: support@hooksniff.vercel.app
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { RequestFile } from './models';
import { DomainDnsRecord } from './domainDnsRecord';

/**
* Result of domain verification attempt
*/
export class VerifyCustomDomainResponse {
    'status': VerifyCustomDomainResponse.StatusEnum;
    /**
    * DNS records that need to be configured
    */
    'dnsRecords': Array<DomainDnsRecord>;

    static discriminator: string | undefined = undefined;

    static attributeTypeMap: Array<{name: string, baseName: string, type: string}> = [
        {
            "name": "status",
            "baseName": "status",
            "type": "VerifyCustomDomainResponse.StatusEnum"
        },
        {
            "name": "dnsRecords",
            "baseName": "dns_records",
            "type": "Array<DomainDnsRecord>"
        }    ];

    static getAttributeTypeMap() {
        return VerifyCustomDomainResponse.attributeTypeMap;
    }
}

export namespace VerifyCustomDomainResponse {
    export enum StatusEnum {
        Verified = <any> 'verified',
        Pending = <any> 'pending',
        Failed = <any> 'failed'
    }
}
