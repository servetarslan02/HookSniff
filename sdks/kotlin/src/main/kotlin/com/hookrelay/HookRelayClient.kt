...
class HookRelayException(
    val statusCode: Int? = null,
    val code: String? = null,
    override val message: String
) : Exception(message)

class AuthenticationException(message: String) : HookRelayException(statusCode = 401, code = "UNAUTHORIZED", message = message)
class ValidationException(message: String) : HookRelayException(statusCode = 400, code = "BAD_REQUEST", message = message)
class NotFoundException(message: String) : HookRelayException(statusCode = 404, code = "NOT_FOUND", message = message)
class RateLimitException(message: String) : HookRelayException(statusCode = 429, code = "RATE_LIMIT", message = message)
class PayloadTooLargeException(message: String) : HookRelayException(statusCode = 413, code = "PAYLOAD_TOO_LARGE", message = message)
...
/**
 * Official Kotlin client for the HookRelay webhook delivery service.
 *
 * Usage:
...
class HookRelayClient(
    ...
    companion object {
        private const val DEFAULT_BASE_URL = "https://api.hookrelay.io/v1"
    ...
    internal inline fun <reified T> request(method: String, path: String, body: Any? = null): T {
        ...
        val requestBuilder = Request.Builder()
            ...
            .header("User-Agent", "hookrelay-kotlin/0.2.0")
        ...
        if (!response.isSuccessful) {
            ...
            throw when (response.code) {
                ...
                else -> HookRelayException(response.code, "UNKNOWN", message)
            }
        }
