=begin
#HookSniff API

#Webhook delivery, monitoring, and management API. All endpoints under `/v1` require authentication via `Authorization: Bearer <api_key>` header unless marked as **Public**. 

The version of the OpenAPI document: 1.0.0
Contact: support@hooksniff.vercel.app
Generated by: https://openapi-generator.tech
Generator version: 7.22.0

=end

require 'spec_helper'
require 'json'

# Unit tests for HooksniffSdk::TemplatesApi
# Automatically generated by openapi-generator (https://openapi-generator.tech)
# Please update as you see appropriate
describe 'TemplatesApi' do
  before do
    # run before each test
    @api_instance = HooksniffSdk::TemplatesApi.new
  end

  after do
    # run after each test
  end

  describe 'test an instance of TemplatesApi' do
    it 'should create an instance of TemplatesApi' do
      expect(@api_instance).to be_instance_of(HooksniffSdk::TemplatesApi)
    end
  end

  # unit tests for templates_get
  # List available templates
  # @param [Hash] opts the optional parameters
  # @option opts [String] :category 
  # @return [Array<WebhookTemplate>]
  describe 'templates_get test' do
    it 'should work' do
      # assertion here. ref: https://rspec.info/features/3-12/rspec-expectations/built-in-matchers/
    end
  end

  # unit tests for templates_id_apply_post
  # Apply template to an endpoint
  # @param id 
  # @param apply_template_request 
  # @param [Hash] opts the optional parameters
  # @return [ApplyTemplateResponse]
  describe 'templates_id_apply_post test' do
    it 'should work' do
      # assertion here. ref: https://rspec.info/features/3-12/rspec-expectations/built-in-matchers/
    end
  end

  # unit tests for templates_id_get
  # Get template by ID
  # @param id 
  # @param [Hash] opts the optional parameters
  # @return [WebhookTemplate]
  describe 'templates_id_get test' do
    it 'should work' do
      # assertion here. ref: https://rspec.info/features/3-12/rspec-expectations/built-in-matchers/
    end
  end

end
