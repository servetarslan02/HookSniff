...
module OpenapiClient
  ...
  class PortalConfig < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::PortalConfig` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::PortalConfig`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'webhook_events')
        if (value = attributes[:'webhook_events']).is_a?(Array)
          self.webhook_events = value
        end
      end
    end
    ...
    def hash
      [logo_url, primary_color, custom_domain, webhook_events].hash
    end
