...
module OpenapiClient
  ...
  class InboundWebhookResponse < ApiModelBase
    attr_accessor :id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::InboundWebhookResponse` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::InboundWebhookResponse`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      status_validator = EnumAttributeValidator.new('String', ["accepted", "rejected", "processing"])
    ...
    def status=(status)
      validator = EnumAttributeValidator.new('String', ["accepted", "rejected", "processing"])
    ...
    def hash
      [id, status, endpoint_id, received_at].hash
    end
