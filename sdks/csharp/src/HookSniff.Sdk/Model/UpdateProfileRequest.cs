...
namespace HookSniff.Sdk.Model
{
    ...
    public class UpdateProfileRequestJsonConverter : JsonConverter<UpdateProfileRequest>
    {
        ...
        public override UpdateProfileRequest Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (!name.IsSet)
                throw new ArgumentException("Property is required for class UpdateProfileRequest.", nameof(name));

            if (!email.IsSet)
                throw new ArgumentException("Property is required for class UpdateProfileRequest.", nameof(email));

            if (name.IsSet && name.Value == null)
                throw new ArgumentNullException(nameof(name), "Property is not nullable for class UpdateProfileRequest.");

            if (email.IsSet && email.Value == null)
                throw new ArgumentNullException(nameof(email), "Property is not nullable for class UpdateProfileRequest.");

            return new UpdateProfileRequest(name.Value!, email.Value!);
        }
        ...
        public void WriteProperties(Utf8JsonWriter writer, UpdateProfileRequest updateProfileRequest, JsonSerializerOptions jsonSerializerOptions)
        {
            if (updateProfileRequest.Name == null)
                throw new ArgumentNullException(nameof(updateProfileRequest.Name), "Property is required for class UpdateProfileRequest.");

            if (updateProfileRequest.Email == null)
                throw new ArgumentNullException(nameof(updateProfileRequest.Email), "Property is required for class UpdateProfileRequest.");

            writer.WriteString("name", updateProfileRequest.Name);

            writer.WriteString("email", updateProfileRequest.Email);
        }
    }
}
