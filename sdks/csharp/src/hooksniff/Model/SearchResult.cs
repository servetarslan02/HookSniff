...
namespace hooksniff.Model
{
    ...
    public partial class SearchResult : IValidatableObject
    {
        ...
        [JsonConstructor]
        public SearchResult(Option<List<Delivery>?> deliveries = default, Option<int?> total = default)
        {
    ...
    public class SearchResultJsonConverter : JsonConverter<SearchResult>
    {
        ...
        public override SearchResult Read(ref Utf8JsonReader utf8JsonReader, Type typeToConvert, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            if (deliveries.IsSet && deliveries.Value == null)
                throw new ArgumentNullException(nameof(deliveries), "Property is not nullable for class SearchResult.");

            if (total.IsSet && total.Value == null)
                throw new ArgumentNullException(nameof(total), "Property is not nullable for class SearchResult.");

            return new SearchResult(deliveries, total);
        }
        ...
        {
            if (searchResult.DeliveriesOption.IsSet && searchResult.Deliveries == null)
                throw new ArgumentNullException(nameof(searchResult.Deliveries), "Property is required for class SearchResult.");

            if (searchResult.DeliveriesOption.IsSet)
            {
                writer.WritePropertyName("deliveries");
                JsonSerializer.Serialize(writer, searchResult.Deliveries, jsonSerializerOptions);
            }
            if (searchResult.TotalOption.IsSet)
                writer.WriteNumber("total", searchResult.TotalOption.Value!.Value);
        }
    }
}
