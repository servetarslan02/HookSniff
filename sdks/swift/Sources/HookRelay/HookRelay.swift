...
/// HookRelay API error.
public struct HookRelayError: Error, LocalizedError {
...
// MARK: - HookRelay Client

/// Official Swift client for the HookRelay webhook delivery service.
///
/// Usage:
...
public class HookRelay: @unchecked Sendable {
    ...
    public init(apiKey: String, baseUrl: String = "https://api.hookrelay.io/v1", timeout: TimeInterval = 30) {
        self.apiKey = apiKey
    ...
    func request<T: Decodable>(_ method: String, path: String, body: [String: Any]? = nil) async throws -> T {
        ...
        request.setValue("hookrelay-swift/0.2.0", forHTTPHeaderField: "User-Agent")
...
public class EndpointsResource: @unchecked Sendable {
    private let client: HookRelay

    init(client: HookRelay) { self.client = client }
