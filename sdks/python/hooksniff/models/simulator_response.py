...
class SimulatorResponse(BaseModel):
    ...
    delivery_id: UUID
    status: StrictStr
    latency_ms: StrictInt = Field(description="Response time from the endpoint")
    __properties: ClassVar[List[str]] = ["delivery_id", "status", "latency_ms"]

    @field_validator('status')
    def status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['delivered', 'failed', 'pending']):
            raise ValueError("must be one of enum values ('delivered', 'failed', 'pending')")
        return value
