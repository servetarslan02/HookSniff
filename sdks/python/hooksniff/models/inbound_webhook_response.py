...
class InboundWebhookResponse(BaseModel):
    ...
    """ # noqa: E501
    id: UUID
    status: StrictStr = Field(description="Processing status of the inbound webhook")
    endpoint_id: UUID
    received_at: datetime
    __properties: ClassVar[List[str]] = ["id", "status", "endpoint_id", "received_at"]

    @field_validator('status')
    def status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['accepted', 'rejected', 'processing']):
            raise ValueError("must be one of enum values ('accepted', 'rejected', 'processing')")
        return value
