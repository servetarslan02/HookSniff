...
class DomainDnsRecord(BaseModel):
    ...
    """ # noqa: E501
    type: StrictStr
    name: StrictStr = Field(description="DNS record name/host")
    value: StrictStr = Field(description="DNS record value")
    status: StrictStr
    __properties: ClassVar[List[str]] = ["type", "name", "value", "status"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['TXT', 'CNAME', 'A']):
            raise ValueError("must be one of enum values ('TXT', 'CNAME', 'A')")
        return value
