...
class UpgradeRequest(BaseModel):
    ...
    __properties: ClassVar[List[str]] = ["plan", "provider"]

    @field_validator('plan')
    def plan_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['pro', 'business']):
            raise ValueError("must be one of enum values ('pro', 'business')")
        return value
    ...
    def provider_validate_enum(cls, value):
        ...
        if value not in set(['stripe', 'polar', 'iyzico']):
            raise ValueError("must be one of enum values ('stripe', 'polar', 'iyzico')")
        return value
