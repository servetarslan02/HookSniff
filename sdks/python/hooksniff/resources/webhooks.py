...
class Webhooks:
    ...
    def send(self, data: Dict[str, Any], idempotency_key: Optional[str] = None) -> Any:
    ...
    def batch(self, data: Dict[str, Any], idempotency_key: Optional[str] = None) -> Any:
        """Send batch webhooks."""
    ...
    def get(self, delivery_id: str) -> Any:
        """Get a specific delivery."""
        req = HookSniffRequest("GET", "/v1/webhooks/{id}")
        req.set_path_param("id", delivery_id)
        return req.send(self._ctx)

    def replay(self, delivery_id: str, idempotency_key: Optional[str] = None) -> Any:
        """Replay a delivery."""
