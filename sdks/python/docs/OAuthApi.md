...
> oauth_github_callback_get(code, state, error=error)
...
with hooksniff.ApiClient(configuration) as api_client:
    ...
    code = 'code_example' # str | Authorization code from GitHub
    state = 'state_example' # str | CSRF state token (verified against cookie)
    error = 'error_example' # str | Error from GitHub (e.g. access_denied) (optional)

    try:
        # GitHub OAuth callback
        api_instance.oauth_github_callback_get(code, state, error=error)
    except Exception as e:
        print("Exception when calling OAuthApi->oauth_github_callback_get: %s\n" % e)
```
 ...
 **state** | **str**| CSRF state token (verified against cookie) | 
 **error** | **str**| Error from GitHub (e.g. access_denied) | [optional] 
...
with hooksniff.ApiClient(configuration) as api_client:
    ...
    try:
        ...
        print("Exception when calling OAuthApi->oauth_github_get: %s\n" % e)
```
