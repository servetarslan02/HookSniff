defmodule HookRelay do
  @moduledoc """
  Official Elixir client for the HookRelay webhook delivery service.
  ...
  ## Usage
      ...
      # Create endpoint
      {:ok, endpoint} = HookRelay.Endpoints.create(client, %{url: "https://myapp.com/webhook"})
  ...
  @user_agent "hookrelay-elixir/0.2.0"
  ...
  def request(%__MODULE__{} = client, method, path, body \\ nil) do
    ...
    case :httpc.request(method_charlist(method), request, http_opts, []) do
      ...
      {:error, reason} ->
        {:error, %HookRelay.Error{message: "HTTP error: #{inspect(reason)}", code: :network_error}}
    end
  end
  ...
  defp parse_error(body, code, status \\ nil) do
    ...
    %HookRelay.Error{message: message, code: code, status: status}
  end
end

defmodule HookRelay.Error do
