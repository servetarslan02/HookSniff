...
$envVarsStr = "APP_ENV=production,RUST_LOG=info,hooksniff=info,LOG_FORMAT=json,CORS_ORIGINS=https://hooksniff.vercel.app,APP_URL=https://hooksniff.vercel.app,POLAR_PRODUCT_PRO=$env:POLAR_PRODUCT_PRO,POLAR_PRODUCT_BUSINESS=$env:POLAR_PRODUCT_BUSINESS,POLAR_ENV=production,RATE_LIMIT_STORE=redis,RETENTION_DAYS=7,WEBHOOK_FORMAT=standard,NOTIFY_FROM_EMAIL=noreply@hooksniff.vercel.app,OTEL_ENABLED=true,OTEL_EXPORTER_OTLP_ENDPOINT=https://otlp-gateway-prod-eu-west-2.grafana.net/otlp,MAX_PAYLOAD_BYTES=1048576,WEBHOOK_TIMESTAMP_TOLERANCE_SECS=300"
...
$workerEnvVars = "APP_ENV=production,RUST_LOG=info,hooksniff=info,NOTIFY_FROM_EMAIL=noreply@hooksniff.vercel.app,OTEL_ENABLED=true,OTEL_EXPORTER_OTLP_ENDPOINT=https://otlp-gateway-prod-eu-west-2.grafana.net/otlp"
...
Write-Host "  Dashboard:   https://hooksniff.vercel.app"
Write-Host "  API Domain:  https://api.hooksniff.vercel.app"
Write-Host ""
Write-Host "Sonraki adimlar:" -ForegroundColor Yellow
Write-Host "  1. Cloudflare DNS: api.hooksniff.vercel.app -> $API_URL CNAME ekle"
