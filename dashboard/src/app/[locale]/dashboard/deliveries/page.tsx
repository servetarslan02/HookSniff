          ...
          <>
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-200/50 dark:divide-slate-700/50">
                {filtered.map((d) => (
                  <tr key={d.id} className="hover:bg-gray-50 dark:hover:bg-slate-800/50 transition cursor-pointer" tabIndex={0} role="link" aria-label={`Delivery ${d.id.slice(0, 12)}`} onClick={() => router.push(`/dashboard/deliveries/${d.id}`)} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); router.push(`/dashboard/deliveries/${d.id}`); } }}>
