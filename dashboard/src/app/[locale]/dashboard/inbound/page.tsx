  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-white">📨 Inbound Webhooks</h2>
          <p className="text-sm text-gray-500 dark:text-slate-400 mt-1">Receive webhooks from Stripe, GitHub, Shopify and route them through HookSniff</p>
        </div>
      ...
      <div className="glass-card p-6 bg-gradient-to-r from-brand-50 to-purple-50 dark:from-brand-500/5 dark:to-purple-500/5">
        ...
        <div className="flex items-center gap-3 text-sm text-gray-600 dark:text-slate-400">
          <span className="px-3 py-1.5 rounded-lg bg-white dark:bg-slate-800 font-mono text-xs">External Service</span>
          <span>→</span>
          <span className="px-3 py-1.5 rounded-lg bg-white dark:bg-slate-800 font-mono text-xs">POST /v1/inbound/:provider</span>
          <span>→</span>
          <span className="px-3 py-1.5 rounded-lg bg-white dark:bg-slate-800 font-mono text-xs">Verify Signature</span>
          <span>→</span>
          <span className="px-3 py-1.5 rounded-lg bg-white dark:bg-slate-800 font-mono text-xs">Your Endpoint</span>
        </div>
      </div>
      ...
      {showCreate && (
        <div className="glass-card p-6 space-y-4">
          <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Add Inbound Provider</h3>
          ...
          {selectedProvider && (
            <>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">Route to Endpoint</label>
              ...
              <div className="flex gap-3">
                <button onClick={handleCreate} className="bg-brand-600 text-white px-6 py-2.5 rounded-xl text-sm font-medium hover:bg-brand-700 transition">Save</button>
                <button onClick={() => setShowCreate(false)} className="bg-gray-100 dark:bg-slate-800 text-gray-700 dark:text-slate-300 px-6 py-2.5 rounded-xl text-sm font-medium hover:bg-gray-200 dark:hover:bg-slate-700 transition">Cancel</button>
              </div>
            </>
          )}
        </div>
      ...
      <div className="glass-card p-6">
        <h3 className="text-sm font-semibold text-gray-900 dark:text-white mb-3">Your Inbound URLs</h3>
        <div className="space-y-2">
          {PROVIDERS.map(p => (
            <div key={p.id} className="flex items-center gap-3">
              ...
              <button onClick={() => navigator.clipboard.writeText(`${API}/inbound/${p.id}`)} className="text-xs text-brand-600 dark:text-brand-400 hover:underline">Copy</button>
            </div>
          ))}
        </div>
      </div>
      ...
      {configs.length > 0 && (
        <div className="glass-card p-6">
          <h3 className="text-sm font-semibold text-gray-900 dark:text-white mb-4">Active Configs</h3>
