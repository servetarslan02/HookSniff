...
export function GuidesContent() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-slate-950">
      <PublicNavbar pageTitle={t("guides")} />
      ...
      <main className="max-w-5xl mx-auto px-6 py-16">
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                {section.items.map((guide) => (
                  <Link
                    key={guide.title}
                    href={guide.href}
                    className="group p-5 bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 hover:border-brand-300 dark:hover:border-brand-500/40 transition-colors"
                  >
        ...
        <div className="mt-16 text-center p-8 bg-gray-900 dark:bg-slate-800 rounded-xl">
          <h2 className="text-2xl font-bold text-white mb-2">{t('readyBuildTitle')}</h2>
          <p className="text-gray-500 dark:text-slate-400 mb-6">{t('readyBuildDesc')}</p>
