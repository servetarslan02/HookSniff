  ...
  return (
    <div className="space-y-4 sm:space-y-6 max-w-3xl">
      ...
      <div className="glass-card p-6">
        <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">✉️ {t('compose') || 'Compose'}</h2>
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-between">
            ...
            >
              {sending ? (t('sending') || 'Sending...') : `📧 ${t('sendBulkEmail') || 'Send Bulk Email'}`}
            </button>
          </div>
        </div>
      </div>
      ...
      {result && (
        <div className="glass-card p-6 border-2 border-emerald-200 dark:border-emerald-500/30">
          <h3 className="text-sm font-semibold text-emerald-700 dark:text-emerald-400 mb-2">✅ {t('sendComplete') || 'Send Complete'}</h3>
      ...
      {history.length > 0 && (
        <div className="glass-card p-6">
          <h3 className="text-sm font-medium text-gray-500 dark:text-slate-400 mb-4">📋 {t('sendHistory') || 'Send History (this session)'}</h3>
            ...
            {history.map((h, i) => (
              <div key={i} className="flex items-center justify-between py-2 px-3 bg-gray-50 dark:bg-slate-800/50 rounded-lg">
                ...
                <div className="flex gap-3 shrink-0 text-xs">
                  ...
                  {h.failed > 0 && <span className="text-red-600 dark:text-red-400">❌ {h.failed}</span>}
                </div>
              </div>
            ))}
          </div>
