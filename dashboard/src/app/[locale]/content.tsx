...
function SocialProof() {
  ...
  const stats = [
    { value: '2.4M+', label: t('webhooksProcessed'), icon: '📨' },
    { value: '12K+', label: t('activeEndpoints'), icon: '🔌' },
    { value: '99.99%', label: t('uptime'), icon: '🛡️' },
    { value: '<50ms', label: t('avgLatency'), icon: '⚡' },
  ];

  const testimonials = [
    { quote: t('testimonial1Quote'), author: t('testimonial1Author'), role: t('testimonial1Role'), avatar: '👩‍💻' },
    { quote: t('testimonial2Quote'), author: t('testimonial2Author'), role: t('testimonial2Role'), avatar: '👨‍🔧' },
    { quote: t('testimonial3Quote'), author: t('testimonial3Author'), role: t('testimonial3Role'), avatar: '👩‍🚀' },
  ];

  const companyLogos = ['Stripe', 'Shopify', 'GitHub', 'Vercel', 'Supabase'];
    ...
    <section className="py-16 border-y border-gray-200/50 dark:border-slate-700/50 bg-white/50 dark:bg-slate-900/50">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-4">
          {testimonials.map((testimonial, i) => (
            <div key={i} className="glass-card p-6 hover-lift">
              ...
              <blockquote className="text-gray-700 dark:text-slate-300 text-sm leading-relaxed mb-4">
                &ldquo;{testimonial.quote}&rdquo;
              </blockquote>
              <div className="flex items-center gap-3">
                <span className="text-2xl" aria-hidden="true">{testimonial.avatar}</span>
                <div>
                  <div className="font-semibold text-gray-900 dark:text-white text-sm">{testimonial.author}</div>
                  <div className="text-xs text-gray-500 dark:text-slate-400">{testimonial.role}</div>
                </div>
              </div>
            </div>
          ))}
        ...
        <div className="text-center">
          <p className="text-sm text-gray-500 dark:text-slate-400 mb-6">{t('companies')}</p>
          <div className="flex flex-wrap items-center justify-center gap-8 opacity-50">
            {companyLogos.map((name) => (
              <span key={name} className="text-lg font-bold text-gray-400 dark:text-slate-500 tracking-wider">
                {name}
              </span>
            ))}
          </div>
