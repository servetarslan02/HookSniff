...
function latencyColor(ms: number | null): string {
  if (ms === null || ms === 0) return 'text-gray-400 dark:text-slate-500';
...
function StatusBadge({ status }: { status: string }) {
  const styles: Record<string, { bg: string; text: string; dot: string; label: string }> = {
    ...
    unknown: { bg: 'bg-gray-50 dark:bg-gray-500/10 border-gray-200 dark:border-gray-500/20', text: 'text-gray-500 dark:text-gray-400', dot: 'bg-gray-400', label: 'Unknown' },
  };
  ...
  return (
    <div className="mt-4">
      ...
      <div className="flex gap-px mt-1" style={{ gap: '2px' }}>
        {months.map((m) => (
          <div key={m.label + m.count} className="text-[10px] text-gray-400 dark:text-slate-500" style={{ width: `${m.count * 14}px` }}>
            {m.label}
          </div>
        ))}
      </div>
              ...
              <div key={inc.id} className="border border-gray-100 dark:border-slate-800 rounded-lg overflow-hidden">
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`text-xs text-gray-400 transition-transform ${expandedId === inc.id ? 'rotate-180' : ''}`}>▼</span>
                  </div>
                </button>
                {expandedId === inc.id && (
                    ...
                    <div className="space-y-2">
                      {inc.updates.map((update, i) => (
                        <div key={i} className="flex gap-3 text-sm">
                          <span className="text-xs text-gray-400 dark:text-slate-500 shrink-0 w-14">
            ...
            {past.map((m) => (
              <div key={m.id} className="flex items-center justify-between py-2 text-sm">
                <div>
                  ...
                  <span className="text-gray-400 dark:text-slate-500 ml-2 text-xs">({m.affected_components.join(', ')})</span>
                </div>
                <span className="text-xs text-gray-400 dark:text-slate-500">{formatDate(m.scheduled_start)}</span>
              </div>
            ))}
          </div>
        </div>
