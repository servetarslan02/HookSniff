      ...
      <main className="max-w-4xl mx-auto px-6 py-16">
        <div className="mb-10">
          <div className="flex items-center gap-3 mb-4">
            ...
            {release.tag === 'latest' && <span className="text-xs font-medium bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 px-2 py-0.5 rounded-full">{t("latest")}</span>}
            <span className="text-xs font-medium bg-gray-100 dark:bg-slate-800 text-gray-600 dark:text-slate-400 px-2 py-0.5 rounded-full">{areaCfg.icon} {areaCfg.label}</span>
            ...
            return (
              <div key={i} className="rounded-xl border border-gray-100 dark:border-slate-700 bg-white dark:bg-slate-800 p-5">
                <div className="flex items-start gap-4">
                  <span className={`inline-flex items-center gap-1.5 text-xs font-medium px-2.5 py-1 rounded-full shrink-0 mt-0.5 ${cfg.bg} ${cfg.color}`}>{cfg.icon} {cfg.label}</span>
                  <div className="flex-1 min-w-0">
                    ...
                    {entry.detail && <p className="text-sm text-gray-600 dark:text-slate-400 mt-2 leading-relaxed">{entry.detail}</p>}
                    {entry.code && <div className="mt-3"><pre className="bg-gray-900 dark:bg-slate-800 rounded-lg p-4 text-xs text-green-400 font-mono overflow-x-auto">{entry.code.snippet}</pre></div>}
                  </div>
                </div>
              </div>
            );
        ...
        <div className="mt-16 pt-8 border-t border-gray-200 dark:border-slate-700 flex items-center justify-between">
          {prevRelease ? (
            <Link href={`/changelog/${prevRelease.slug}`} className="group">
              ...
              <p className="text-sm font-medium text-gray-700 dark:text-slate-300 group-hover:text-brand-600 dark:group-hover:text-brand-400 transition-colors">{prevRelease.version} — {prevRelease.title}</p>
            </Link>
          ) : <div />}
          {nextRelease ? (
            <Link href={`/changelog/${nextRelease.slug}`} className="group text-right">
              ...
              <p className="text-sm font-medium text-gray-700 dark:text-slate-300 group-hover:text-brand-600 dark:group-hover:text-brand-400 transition-colors">{nextRelease.version} — {nextRelease.title}</p>
            </Link>
          ) : <div />}
        </div>
