  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-gray-500 dark:text-slate-400 mt-1">Search and filter your webhook delivery logs.</p>
      </div>
      ...
      <div className="glass-card overflow-hidden">
        {isLoading ? (
          ...
          <>
            <div className="px-6 py-3 border-b border-gray-200/50 dark:border-slate-700/50">
              <span className="text-sm text-gray-500 dark:text-slate-400">{total.toLocaleString()} result{total !== 1 ? 's' : ''}</span>
            </div>
                ...
                <tbody className="divide-y divide-gray-200/50 dark:divide-slate-700/50">
                  {deliveries.map((d) => (
                    <tr key={d.id} className="hover:bg-gray-50/50 dark:hover:bg-slate-800/50 transition cursor-pointer" onClick={() => router.push(`/deliveries/${d.id}`)}>
                      ...
                      <td className="px-6 py-4"><span className="inline-flex items-center px-2.5 py-0.5 rounded-md bg-gray-100 dark:bg-slate-700 text-xs font-mono text-gray-700 dark:text-slate-300">{d.event || '—'}</span></td>
                      <td className="px-6 py-4"><StatusBadge status={d.status} /></td>
                      <td className="px-6 py-4 text-sm text-gray-500 dark:text-slate-400 font-mono max-w-[200px] truncate">{d.endpoint_url}</td>
                      <td className="px-6 py-4 text-sm text-gray-600 dark:text-slate-400">{d.attempt_count}{d.response_status && <span className="ml-1 text-xs text-gray-500 dark:text-slate-400">({d.response_status})</span>}</td>
                      <td className="px-6 py-4 text-sm text-gray-500 dark:text-slate-500">{new Date(d.created_at).toLocaleString()}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            {totalPages > 1 && (
              <nav aria-label={tc('pagination')} className="px-6 py-4 border-t border-gray-200/50 dark:border-slate-700/50 flex items-center justify-between">
                <button type="button" onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1} className="px-4 py-2 text-sm border border-gray-300 dark:border-slate-600 rounded-lg disabled:opacity-50 transition">{tc('previous')}</button>
                <span className="text-sm text-gray-500 dark:text-slate-400">{tc('pageOf', { page, total: totalPages })}</span>
                <button type="button" onClick={() => setPage((p) => p + 1)} disabled={page >= totalPages} className="px-4 py-2 text-sm border border-gray-300 dark:border-slate-600 rounded-lg disabled:opacity-50 transition">{tc('next')}</button>
              </nav>
            )}
          </>
        )}
