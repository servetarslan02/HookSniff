     ...
     <div key={tpl.id} className="glass-card p-5 hover:shadow-lg transition flex flex-col">
       <div className="flex-1">
        <div className="flex items-start justify-between mb-2">
         ...
         {tpl.industry && <span className="px-2 py-0.5 text-xs font-medium bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-slate-400 rounded-md shrink-0 ml-2">{tpl.industry}</span>}
        </div>
        <p className="text-xs text-gray-500 dark:text-slate-400 mb-3 line-clamp-2">{getTemplateDesc(tpl.id) || tpl.description}</p>
        {tpl.tags && tpl.tags.length > 0 && <div className="flex flex-wrap gap-1 mb-3">{tpl.tags.slice(0, 3).map((tag) => <span key={tag} className="px-2 py-0.5 text-xs bg-brand-50 dark:bg-brand-500/10 text-brand-700 dark:text-brand-400 rounded-md">{tag}</span>)}{tpl.tags.length > 3 && <span className="px-1 text-xs text-gray-400">+{tpl.tags.length - 3}</span>}</div>}
        ...
        <div className="flex flex-wrap gap-1 mb-3">{tpl.event_types.slice(0, 3).map((ev) => <span key={ev} className="px-1.5 py-0.5 text-xs font-mono bg-gray-50 dark:bg-slate-800 text-gray-600 dark:text-slate-400 rounded border border-gray-200 dark:border-slate-700">{ev}</span>)}{tpl.event_types.length > 3 && <span className="px-1.5 py-0.5 text-xs text-gray-400">+{tpl.event_types.length - 3}</span>}</div>
       </div>
       <div className="flex items-center gap-2 pt-3 border-t border-gray-100 dark:border-slate-700">
        <button onClick={() => setSelectedTemplate(tpl)} className="flex-1 px-3 py-2 text-xs font-medium text-gray-700 dark:text-slate-300 bg-gray-100 dark:bg-slate-700 hover:bg-gray-200 dark:hover:bg-slate-600 rounded-lg transition">Details</button>
        <button onClick={() => openApply(tpl)} className="flex-1 px-3 py-2 text-xs font-medium text-white bg-brand-600 hover:bg-brand-700 rounded-lg transition">Apply</button>
       </div>
     </div>
    ))}
   </div>
