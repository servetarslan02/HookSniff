        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={closeModal}>
          ...
          >
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-slate-800">
              <h2 className="text-lg font-semibold text-gray-900 dark:text-white">{t('editRouting') || 'Edit Routing'}</h2>
            ...
            <div className="px-6 py-5 space-y-5">
              <div className="flex items-center gap-2">
                <code className="text-sm font-mono text-gray-700 dark:text-slate-300 flex-1 truncate">{editingEndpoint.url}</code>
                <span className={`px-2 py-0.5 rounded-full text-xs shrink-0 ${editingEndpoint.is_active ? 'bg-green-100 dark:bg-green-500/20 text-green-700 dark:text-green-400' : 'bg-gray-100 dark:bg-slate-700 text-gray-500 dark:text-slate-400'}`}>
                  {editingEndpoint.is_active ? tc('active') : tc('inactive')}
                </span>
              </div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {STRATEGIES.map((s) => (
                    ...
                    >
                      ...
                      <div className="text-xs text-gray-500 dark:text-slate-400 mt-0.5">{t(s.descKey) || ''}</div>
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2.5 border border-gray-200 dark:border-slate-600 rounded-xl bg-white dark:bg-slate-800 text-gray-900 dark:text-white text-sm font-mono focus:ring-2 focus:ring-brand-500/30 focus:border-brand-500 outline-none transition"
                />
                <p className="text-xs text-gray-400 dark:text-slate-500 mt-1.5">{t('fallbackHint') || 'Used when primary endpoint fails (failover strategy)'}</p>
              </div>
            </div>

            {/* Footer */}
            <div className="flex gap-2 justify-end px-6 py-4 border-t border-gray-100 dark:border-slate-800">
