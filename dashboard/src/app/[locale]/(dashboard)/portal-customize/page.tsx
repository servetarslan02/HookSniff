...
export default function PortalCustomizationPage() {
  ...
  const { data: configData, isLoading, error: configError, refetch } = usePortalConfig();
  ...
  const handleSave = () => {
    updateConfig.mutate(config as Partial<PortalConfigResponse>, {
      ...
      onError: (err) => {
        const msg = err instanceof Error ? err.message : t('portalSaveFailed');
        toast(msg, 'error');
      },
    });
  };
  ...
  return (
    <div className="max-w-6xl space-y-8">
      {/* Load error warning — non-blocking, page stays usable */}
      {loadFailed && (
            ...
            <div className="flex flex-wrap gap-2">
              {config.allowed_events.map((event) => (
                ...
                >
                  ...
                  >
                    <X size={16} strokeWidth={1.75} className="inline mr-1" />
                  </button>
                </span>
              ))}
