...
async function InboundWebhooksPageContent(params: Promise<{ locale: string }>) {
  ...
  "endpoint_id": "ep_abc123"
        ...
        <div className="not-prose overflow-hidden rounded-xl border border-gray-200 dark:border-slate-700">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-slate-700">
              <tr><td className="px-4 py-3 text-gray-600 dark:text-slate-400">Stripe</td><td className="px-4 py-3 font-mono text-gray-600 dark:text-slate-400">Stripe-Signature</td><td className="px-4 py-3 text-gray-600 dark:text-slate-400"><CheckCircle2 size={14} className="text-emerald-500" /></td></tr>
              <tr><td className="px-4 py-3 text-gray-600 dark:text-slate-400">GitHub</td><td className="px-4 py-3 font-mono text-gray-600 dark:text-slate-400">X-Hub-Signature-256</td><td className="px-4 py-3 text-gray-600 dark:text-slate-400"><CheckCircle2 size={14} className="text-emerald-500" /></td></tr>
              <tr><td className="px-4 py-3 text-gray-600 dark:text-slate-400">Shopify</td><td className="px-4 py-3 font-mono text-gray-600 dark:text-slate-400">X-Shopify-Hmac-SHA256</td><td className="px-4 py-3 text-gray-600 dark:text-slate-400"><CheckCircle2 size={14} className="text-emerald-500" /></td></tr>
            </tbody>
          </table>
        </div>
      </section>
      ...
      <section className="mb-12">
        ...
        <ul className="space-y-2">
          ...
          <li className="text-gray-600 dark:text-slate-400"><CheckCircle2 size={14} className="inline mr-1 text-emerald-500 align-text-bottom" />{t('benefit5')}</li>
        </ul>
      </section>
    </article>
  );
...
export default async function Page(params: Promise<{ locale: string }>) {
  return (
    <Suspense fallback={<div className="animate-pulse space-y-4"><div className="h-8 w-64 rounded bg-gray-200 dark:bg-gray-700" /><div className="h-4 w-full rounded bg-gray-200 dark:bg-gray-700" /><div className="h-4 w-3/4 rounded bg-gray-200 dark:bg-gray-700" /><div className="h-64 w-full rounded bg-gray-200 dark:bg-gray-700" /></div>}>
      <InboundWebhooksPageContent {...params} />
    </Suspense>
  );
}
