...
describe('StatusBadge', () => {
  ...
  it('falls back to pending style for unknown status', () => {
    const { container } = renderWithProviders(<StatusBadge status="unknown_status" />);
    const badge = container.querySelector('span')!;
    expect(badge.classList.contains('bg-amber-50')).toBe(true);
  });
  ...
  it('renders with sm size', () => {
    const { container } = renderWithProviders(<StatusBadge status="active" size="sm" />);
    const badge = container.querySelector('span')!;
    expect(badge.classList.contains('text-xs')).toBe(true);
  });

  it('renders with lg size', () => {
    const { container } = renderWithProviders(<StatusBadge status="active" size="lg" />);
    const badge = container.querySelector('span')!;
    expect(badge.classList.contains('text-sm')).toBe(true);
  });

  it('applies custom className', () => {
    const { container } = renderWithProviders(<StatusBadge status="active" className="extra" />);
    const badge = container.querySelector('span')!;
