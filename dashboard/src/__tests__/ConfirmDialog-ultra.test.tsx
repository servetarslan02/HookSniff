...
describe('ConfirmDialog-ultra', () => {
  ...
  it('renders when open is true', () => {
    const { getByText } = renderWithProviders(<ConfirmDialog {...defaultProps} />);
  ...
  it('calls onConfirm when confirm button clicked', () => {
    ...
    const { getByText } = renderWithProviders(<ConfirmDialog {...defaultProps} onConfirm={onConfirm} />);
  ...
  it('calls onCancel when cancel button clicked', () => {
    ...
    const { getByText } = renderWithProviders(<ConfirmDialog {...defaultProps} onCancel={onCancel} />);
  ...
  it('shows "Processing..." when loading', () => {
    const { getByText } = renderWithProviders(<ConfirmDialog {...defaultProps} loading={true} />);
  ...
  it('applies danger variant class to confirm button', () => {
    const { getByText } = renderWithProviders(<ConfirmDialog {...defaultProps} variant="danger" />);
  ...
  it('applies default variant class to confirm button', () => {
    const { getByText } = renderWithProviders(<ConfirmDialog {...defaultProps} variant="default" />);
    const confirmBtn = getByText('Confirm').closest('button')!;
  ...
  it('has dialog role attribute', () => {
    const { getByRole } = renderWithProviders(<ConfirmDialog {...defaultProps} />);
  ...
  it('restores body overflow when dialog closes', () => {
    const { rerender } = renderWithProviders(<ConfirmDialog {...defaultProps} />);
    expect(document.body.style.overflow).toBe('hidden');
