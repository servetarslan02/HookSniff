...
describe('SendWebhookPage — Extended Coverage', () => {
  ...
  it('allows typing event type', async () => {
    const { container } = render(React.createElement(SendWebhookPage));
    const eventTypeInput = container.querySelector('input[placeholder*="event"]') as HTMLInputElement ||
    ...
    expect(eventTypeInput.value).toBe('order.created');
  });
  ...
  it('shows loading spinner during send', async () => {
    ...
    await act(async () => {
      resolveCreate!({ id: 'wh_done', status: 'delivered' });
    });
  });
